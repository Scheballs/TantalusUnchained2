...
{
    public ItemResourceScanReport() {super(new Item.Properties().maxStackSize(65).group(TantalusUnchained.CREATIVE_TAB));
    }
        ...
        if(isHoldingShift()) {
            if (stack.hasTag() && stack.getTag().contains(TantalusUnchained.MOD_ID + "hard_water_density")) {
                ...
                tooltip.add(new StringTextComponent("Hard Water Density on Chunk (x, y): "
                        +String.format("%,.0f", stack.getTag().getDouble(TantalusUnchained.MOD_ID +"hard_water_density") * 100)
                        )
                );
            }
        }
